...
function getTransporter() {
    const user = config_1.config.mail.user;
    const pass = config_1.config.mail.pass;
    const hasAuth = Boolean(user && pass);
    const explicitService = config_1.config.mail.service;
    const host = config_1.config.mail.host;
    const inferredGmailService = !explicitService && !host && hasAuth && user.toLowerCase().endsWith('@gmail.com');
    const effectiveService = explicitService || (inferredGmailService ? 'gmail' : '');
    if (!hasAuth || (!effectiveService && !host)) {
        return null;
    }
    if (!transporter) {
        transporter = nodemailer_1.default.createTransport(effectiveService
            ? {
        ...
        resolvedServiceHint = effectiveService ? `service=${effectiveService}` : `host=${host}:${config_1.config.mail.port}`;
    }
    return transporter;
}
async function sendInterviewScheduleEmail(input) {
    try {
        const tx = getTransporter();
        if (!tx) {
            const err = 'Mail is not configured. Set either (MAIL_SERVICE + MAIL_USER + MAIL_PASS) or (MAIL_HOST + MAIL_PORT + MAIL_USER + MAIL_PASS).';
    ...
    `;
        await tx.sendMail({
            ...
            to: input.to,
            replyTo: config_1.config.mail.replyTo || undefined,
            subject,
