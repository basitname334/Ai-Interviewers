...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      const { candidateEmail, candidateName, role, scheduledAt, positionId, message, difficulty, customQuestions, codingQuestions, focusAreas, durationMinutes } = req.body as {
      ...
      const { rows } = await query<{
        ...
        `INSERT INTO scheduled_interviews (id, candidate_email, candidate_name, role, preferred_difficulty, custom_questions, focus_areas, duration_minutes, scheduled_at, join_token, position_id, created_by, created_at, updated_at)
