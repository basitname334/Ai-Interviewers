...
exports.scoringEngineService = exports.ScoringEngineService = void 0;
...
class ScoringEngineService {
    ...
    async scoreAnswer(question, answer) {
        try {
            ...
            const evaluation = await llm_service_1.llmService.evaluateAnswer(question, answer);
            ...
            logger_1.logger.error('Failed to score answer', { error, question });
            throw error;
        }
    }
    /**
     ...
     */
    async calculateOverallScore(questionScores, transcript) {
        try {
            // Calculate average scores
            const avgCommunication = this.average(questionScores.map((s) => s.communicationScore));
            const avgTechnical = this.average(questionScores.map((s) => s.technicalScore));
            const avgRelevance = this.average(questionScores.map((s) => s.relevanceScore));
            const avgConfidence = this.average(questionScores.map((s) => s.confidenceScore));
            const avgStructuredThinking = this.average(questionScores.map((s) => s.structuredThinkingScore));
            // Calculate weighted overall score
            const overallScore = Math.round(avgCommunication * 0.2 +
                avgTechnical * 0.3 +
                avgRelevance * 0.2 +
                avgConfidence * 0.15 +
                avgStructuredThinking * 0.15);
            // Generate comprehensive feedback using LLM
            const feedback = await llm_service_1.llmService.generateInterviewSummary(transcript, {
            ...
            logger_1.logger.error('Failed to calculate overall score', { error });
            throw error;
        }
    }
    /**
    ...
    evaluateCommunication(answer) {
        ...
        const hasConjunctions = conjunctions.some(word => answer.toLowerCase().includes(word));
        if (hasConjunctions) {
            score += 10;
        }
    ...
    evaluateRelevance(question, answer) {
        ...
        const matchedTerms = questionWords.filter(word => answerLower.includes(word));
        ...
        const hasDirectResponse = directIndicators.some(phrase => answerLower.includes(phrase));
    ...
    evaluateStructure(answer) {
        ...
        const hasProblemSolution = (answerLower.includes('problem') || answerLower.includes('challenge')) &&
...
exports.scoringEngineService = new ScoringEngineService();
