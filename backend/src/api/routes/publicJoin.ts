...
router.post(
  ...
  async (req: Request, res: Response) => {
    const token = req.params.token;
    const { rows } = await query(
      `SELECT id, candidate_email, candidate_name, role, preferred_difficulty, custom_questions, focus_areas, duration_minutes, position_id, application_id, status, interview_id
       FROM scheduled_interviews WHERE join_token = $1`,
      [token]
    );
    if (rows.length === 0) {
